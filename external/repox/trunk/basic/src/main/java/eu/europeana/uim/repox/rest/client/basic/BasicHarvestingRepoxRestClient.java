 ...
 * @since 10.02.2013
 */
public class BasicHarvestingRepoxRestClient extends AbstractRepoxRestClient
		implements HarvestingRepoxRestClient {
	/**
	...
	public String cancelHarvesting(String dsID) throws RepoxException {
		...
		Response resp = invokeRestCall("/dataSources/stopIngest",
				Response.class, id.toString());

		if (resp.getSuccess() == null) {
			if (resp.getError() != null) {
				throw new RepoxException(
						"Could not cancel harvesting! Reason: "
								+ resp.getError().getCause());
			} else {
				throw new RepoxException(
						"Unidentified repox error during canceling of harvesting!");
			}
		} else {
			return resp.getSuccess();
		}
	...
	public String scheduleHarvesting(String dsID, Date ingestionDate,
		...
		firstRunDate.append(calendar.get(Calendar.DAY_OF_WEEK) + "-"
				+ calendar.get(Calendar.MONTH) + "-"
				+ calendar.get(Calendar.YEAR));

		firstRunHour.append("firstRunHour=");
		firstRunHour.append(calendar.get(Calendar.HOUR) + ":"
		...
		Response resp = invokeRestCall("/dataSources/scheduleIngest",
		...
		if (resp.getSuccess() == null) {
			if (resp.getError() != null) {
				throw new RepoxException(
						"Could not schedule harvesting! Reason: "
								+ resp.getError().getCause());
			} else {
				throw new RepoxException(
	...
	public HarvestingStatus getHarvestingStatus(String dsID)
		...
		Response resp = invokeRestCall("/dataSources/harvestStatus",
				Response.class, id.toString());

		if (resp.getHarvestingStatus() == null) {
			if (resp.getError() != null) {
				throw new RepoxException(
						"Could not retrieve harvesting status! Reason: "
								+ resp.getError().getCause());
			} else {
				throw new RepoxException(
	...
	public RunningTasks getActiveHarvestingSessions() throws RepoxException {
		Response resp = invokeRestCall("/dataSources/harvesting",
				Response.class);

		if (resp.getRunningTasks() == null) {
			if (resp.getError() != null) {
				throw new RepoxException(
						"Could not retrieve active harvesting sessions! Reason: "
								+ resp.getError().getCause());
			} else {
				throw new RepoxException(
	...
	public ScheduleTasks getScheduledHarvestingSessions(String dsID)
		...
		Response resp = invokeRestCall("/dataSources/scheduleList",
				Response.class, id.toString());
	...
	public Log getHarvestLog(String dsID) throws RepoxException {
		...
		Response resp = invokeRestCall("/dataSources/log", Response.class,
				id.toString());

		if (resp.getLog() == null) {
			if (resp.getError() != null) {
				throw new RepoxException(
						"Could not retrieve harvesting log! Reason: "
								+ resp.getError().getCause());
			} else {
				throw new RepoxException(
