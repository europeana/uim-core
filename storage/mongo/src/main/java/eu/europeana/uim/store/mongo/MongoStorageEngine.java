...
public class MongoStorageEngine extends AbstractEngine implements StorageEngine<String> {
	...
	// private static THashMap<String, THashSet<String>>
	...
	public void initialize() {
		try {
			...
			morphia.map(MongoProviderDecorator.class).map(MongoExecutionDecorator.class)
					.map(MongoCollectionDecorator.class).map(MongoRequestDecorator.class)
					.map(MongoMetadataRecordDecorator.class).map(MDRPerCollectionAggregator.class)
					.map(MDRPerRequestAggregator.class);
			morphia.getMapper().getOptions().setObjectFactory(new DefaultCreator() {
				@Override
				protected ClassLoader getClassLoaderForClass(String clazz, DBObject object) {
					// we're the only bundle for now using Morphia so we can be
					// sure that in any case
	...
	private void ensureIndexes() {
		...
		mongo.getDB(DEFAULT_UIM_DB_NAME).getCollection("MongoProviderDecorator").ensureIndex(NAMEFIELD);
		mongo.getDB(DEFAULT_UIM_DB_NAME).getCollection("MongoProviderDecorator").ensureIndex(MNEMONICFIELD);
		mongo.getDB(DEFAULT_UIM_DB_NAME).getCollection("MongoCollectionDecorator").ensureIndex(NAMEFIELD);
		mongo.getDB(DEFAULT_UIM_DB_NAME).getCollection("MongoCollectionDecorator").ensureIndex(MNEMONICFIELD);
		mongo.getDB(DEFAULT_UIM_DB_NAME).getCollection("MongoRequestDecorator").ensureIndex(SEARCHDATE);
		mongo.getDB(DEFAULT_UIM_DB_NAME).getCollection("MongoRequestDecorator").ensureIndex(COLLECTIONID);
		mongo.getDB(DEFAULT_UIM_DB_NAME).getCollection("MongoMetadataRecordDecorator").ensureIndex(COLLECTIONID);
		mongo.getDB(DEFAULT_UIM_DB_NAME).getCollection("MongoMetadataRecordDecorator").ensureIndex(RECORDUIDFIELD);
		mongo.getDB(DEFAULT_UIM_DB_NAME).getCollection("MDRPerCollectionAggregator").ensureIndex(COLLECTIONID);
		mongo.getDB(DEFAULT_UIM_DB_NAME).getCollection("MDRPerRequestAggregator").ensureIndex(REQUESTID);
	}
	...
	public void updateProvider(Provider<String> provider) throws StorageEngineException {
		...
		ArrayList<MongoProviderDecorator> result1 = (ArrayList<MongoProviderDecorator>) ds
				.find(MongoProviderDecorator.class).filter(NAMEFIELD, provider.getName()).asList();
		ArrayList<MongoProviderDecorator> result2 = (ArrayList<MongoProviderDecorator>) ds
				.find(MongoProviderDecorator.class).filter(MNEMONICFIELD, provider.getMnemonic()).asList();
		...
		for (MongoProviderDecorator<String> p : allresults) {
			if (p.getName() != null
					&& (p.getName().equals(provider.getName()) || p.getMnemonic().equals(provider.getMnemonic()))
					&& !p.getId().equals(provider.getId())) {
				throw new StorageEngineException("Provider with name '" + provider.getName() + "' already exists");
			}
			if (p.getMnemonic() != null && p.getMnemonic().equals(provider.getMnemonic())
					&& !p.getId().equals(provider.getId())) {
				throw new StorageEngineException(
						"Provider with mnemonic '" + provider.getMnemonic() + "' already exists");
			}
		}
	...
	public Provider<String> findProvider(String mnemonic) {
		MongoProviderDecorator<String> prov = ds.find(MongoProviderDecorator.class).field(MNEMONICFIELD).equal(mnemonic)
				.get();
	...
	public void updateCollection(Collection<String> collection) throws StorageEngineException {
		...
		ArrayList<MongoCollectionDecorator> result1 = (ArrayList<MongoCollectionDecorator>) ds
				.find(MongoCollectionDecorator.class).filter(NAMEFIELD, collection.getName()).asList();
		ArrayList<MongoCollectionDecorator> result2 = (ArrayList<MongoCollectionDecorator>) ds
				.find(MongoCollectionDecorator.class).filter(MNEMONICFIELD, collection.getMnemonic()).asList();
		...
		for (Collection<String> c : allresults) {
			if (c.getName() != null && (c.getName().equals(collection.getName()))
				...
				throw new StorageEngineException(
						"Collection with name '" + collection.getMnemonic() + "' already exists");
			}
			if (c.getMnemonic() != null && c.getMnemonic().equals(collection.getMnemonic())
					&& !c.getId().equals(collection.getId())) {
				throw new StorageEngineException(
						"Collection with mnemonic '" + collection.getMnemonic() + "' already exists");
			}

		}
		inmemoryCollections.put(collection.getId(), (MongoCollectionDecorator<String>) collection);
		ds.merge(collection);
	}
	...
	public Collection<String> getCollection(String id) {
		...
		if (coll == null) {
			...
			coll = ds.find(MongoCollectionDecorator.class).filter(LOCALIDFIELD, objid).get();
	...
	public Collection<String> findCollection(String mnemonic) {
		@SuppressWarnings("rawtypes")
		MongoCollectionDecorator coll = ds.find(MongoCollectionDecorator.class).filter(MNEMONICFIELD, mnemonic).get();
	...
	public List<Collection<String>> getCollections(Provider<String> provider) {
		...
		for (MongoCollectionDecorator<String> c : ds.find(MongoCollectionDecorator.class).filter("provider", provider)
				.asList()) {
	...
	public List<Collection<String>> getAllCollections() {
		List<Collection<String>> res = new ArrayList<Collection<String>>();
		for (MongoCollectionDecorator<String> c : ds.find(MongoCollectionDecorator.class).asList()) {
			res.add(c.getEmbeddedCollection());
		}
		return res;
	}
	...
	public Request<String> getRequest(String id) throws StorageEngineException {
		MongoRequestDecorator<?> req = ds.find(MongoRequestDecorator.class).filter(LOCALIDFIELD, new ObjectId(id))
				.get();

		@SuppressWarnings("rawtypes")
		MongoCollectionDecorator coll = (MongoCollectionDecorator<?>) ensureConsistency(
				getCollection(req.getCollectionID()));
	 ...
	 */
	@Override
	public void addRequestRecord(Request<String> request, MetaDataRecord<String> record) throws StorageEngineException {

		MongoMetadataRecordDecorator<String> rec = (MongoMetadataRecordDecorator<String>) record;

		THashSet<String> registeredRequestrecords = inmemoryRequestRecordIDs.get(request.getId());
	...
	@SuppressWarnings("unchecked")
	@Override
	public MetaDataRecord<String> createMetaDataRecord(Collection<String> collection, String identifier)
	...
	public void updateMetaDataRecord(MetaDataRecord<String> record) throws StorageEngineException {
		...
		// THashSet<String> registeredCollectionrecords =
		// inmemoryCollectionRecordIDs
	...
	public List<Execution<String>> getAllExecutions() {
		...
		for (MongoExecutionDecorator<String> e : ds.find(MongoExecutionDecorator.class).asList()) {
	...
	public MetaDataRecord<String> getMetaDataRecord(String id) throws StorageEngineException {
		@SuppressWarnings("unchecked")
		MongoMetadataRecordDecorator<String> mdr = ds.find(MongoMetadataRecordDecorator.class)
				.filter(RECORDUIDFIELD, id).get();

		if (mdr != null) {
			@SuppressWarnings("unchecked")
			MongoCollectionDecorator<String> coll = (MongoCollectionDecorator<String>) ensureConsistency(
					getCollection(mdr.getCollectionID()));
	...
	public String[] getByRequest(Request<String> request) {
		...
		if (idrefs != null) {
			...
			MDRPerRequestAggregator aggregator = ds.find(MDRPerRequestAggregator.class)
	...
	public String[] getByCollection(Collection<String> col) {
		...
		// no reason to execute 2 queries (one just for counting) since this is
		// just a cursor..
		// int count
		// =ds.getMongo().getDB("UIM").getCollection("MongoMetadataRecordDecorator").find(new
		// BasicDBObject("collectionID", col.getId())).count();
		Iterator<DBObject> obj = ds.getMongo().getDB("UIM").getCollection("MongoMetadataRecordDecorator")
				.find(new BasicDBObject("collectionID", col.getId()), new BasicDBObject("uniqueID", 1));
		while (obj.hasNext()) {
		...
		if (keyCriteriaPostFilters.isEmpty()) {
			...
			for (KeyCriterium<?, ?> keyCriterium : keyCriteriaMongoFilters) {
				if (keyCriterium.isNot()) {
					basicDBObject.append(
							MongoMetadataRecordDecorator.keysExposedAsMongoFields.get(keyCriterium.getKey()),
							new BasicDBObject("$ne", keyCriterium.getValue().getClass().isEnum()
									? keyCriterium.getValue().toString() : keyCriterium.getValue()));
				} else {
					basicDBObject.append(
							MongoMetadataRecordDecorator.keysExposedAsMongoFields.get(keyCriterium.getKey()),
							keyCriterium.getValue().getClass().isEnum() ? keyCriterium.getValue().toString()
									: keyCriterium.getValue());
				}
			}
			Iterator<DBObject> obj = ds.getMongo().getDB("UIM").getCollection("MongoMetadataRecordDecorator")
					.find(basicDBObject, new BasicDBObject("uniqueID", 1));
			while (obj.hasNext()) {
				res.add(obj.next().get("uniqueID").toString());
			}
		} else {
			...
			for (KeyCriterium<?, ?> keyCriterium : keyCriteriaMongoFilters) {
				if (keyCriterium.isNot()) {
					...
					query.filter(MongoMetadataRecordDecorator.keysExposedAsMongoFields.get(keyCriterium.getKey()),
							keyCriterium.getValue().getClass().isEnum() ? keyCriterium.getValue().toString()
									: keyCriterium.getValue());
				}
			}
			...
			MongoMetadataRecordDecorator mongoMetadataRecordDecorator : it) {
				boolean hit = true;
				for (KeyCriterium<?, ?> keyCriterium : keyCriteriaPostFilters) {
					@SuppressWarnings("unchecked")
					List<?> values = mongoMetadataRecordDecorator.getValues(keyCriterium.getKey(),
							keyCriterium.getQualifiers());
					if (keyCriterium.isNot()) {
	...
	@Override
	public int countByCollectionAndCriteria(Collection<String> collection, KeyCriterium<?, ?>... keyCriteria)
			throws StorageEngineException {
		if (keyCriteria == null) {
			return getTotalByCollection(collection);
		}
		List<KeyCriterium<?, ?>> keyCriteriaMongoFilters = new ArrayList<>();
		List<KeyCriterium<?, ?>> keyCriteriaPostFilters = new ArrayList<>();
		...
		if (keyCriteriaPostFilters.isEmpty()) {
			...
			return  ds.getMongo().getDB("UIM").getCollection("MongoMetadataRecordDecorator")
					.find(basicDBObject, new BasicDBObject("uniqueID", 1)).count();
			
		} else {
			String[] ids = getByCollectionAndCriteria(collection, keyCriteria);
	...
	public String[] getByProvider(Provider<String> provider, boolean recursive) {
		...
		List<MongoCollectionDecorator> mongoCollections = ds.find(MongoCollectionDecorator.class)
	...
	public int getTotalByCollection(Collection<String> collection) {
		return ds.getMongo().getDB("UIM").getCollection("MongoMetadataRecordDecorator")
		.find(new BasicDBObject("collectionID", collection.getId()), new BasicDBObject("uniqueID", 1)).count();
	}
	...
	public int getTotalForAllIds() {
		...
		@SuppressWarnings("rawtypes")
		List<MongoCollectionDecorator> mongoCollections = ds.find(MongoCollectionDecorator.class).asList();
	...
	public Execution<String> getExecution(String id) throws StorageEngineException {
		MongoExecutionDecorator exec = ds.find(MongoExecutionDecorator.class, LOCALIDFIELD, new ObjectId(id)).get();
	...
	public void flushRequestMDRS(String requestID) {
		...
		if (recids != null) {
			...
			MDRPerRequestAggregator aggregator = ds.find(MDRPerRequestAggregator.class).filter("requestId", requestID)
					.get();
			if (aggregator == null) {
	...
	/**
	 * This synchronized method is called when the amount of the collections
	 * that have their records cached into memory exceeds the upper limit (which
	 * is 50)
	 */
	public synchronized void purgeInmemoryRequestRecordIDs() {
		...
		if (inmemoryRequestRecordIDs.size() > MAXINMEMORYALLOWED) {
			...
			@SuppressWarnings("unchecked")
			List<ActiveExecution<?, String>> activeExecs = (List<ActiveExecution<?, String>>) orchestrator
					.getActiveExecutions();
			...
			synchronized (inmemoryRequestRecordIDs) {
				inmemoryRequestRecordIDs
						.forEachEntry(new InmemoryRecordIDITerator(content2bepreserved, inmemoryRequestRecordIDs));
			}
		}
	}
	...
	private class InmemoryRecordIDITerator implements TObjectObjectProcedure<String, THashSet<String>> {
		...
		public InmemoryRecordIDITerator(Set<String> tobepresernved,
				THashMap<String, THashSet<String>> structuretobeprocessed) {
		 ...
		 * 
		 * @see
		 * gnu.trove.procedure.TObjectObjectProcedure#execute(java.lang.Object,
		 * java.lang.Object)
		 */
		@Override
