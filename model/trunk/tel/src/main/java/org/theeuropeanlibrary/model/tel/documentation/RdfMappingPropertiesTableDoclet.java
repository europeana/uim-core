...
public class RdfMappingPropertiesTableDoclet {
    /**
     * @param root
     * @return success
     */
...
// csv.append("Class;Field type;Field;EDM-property;Non-EDM property\n");
        csv.append("Class;Field type;Field;edm:ProvidedCHO;edm:WebResource;ore:Aggregation;EDM properties;edm:TimeSpan;edm:Place;edm:Agent;skos:Concept;EDM properties;Notes on mapping\n");
        ...
        try {
            FileUtils.write(new File("target/ObjModelPropertiesToRdf.csv"), csv.toString());
        } catch (Exception e) {
    ...
    private static void classToCsv(StringBuffer csv, ClassDoc cls, HashSet<String> allClasseNames,
        ...
        csv.append(String.format("%s;class\n", overideClassName));
    ...
    private static void qualifierToCsv(StringBuffer csv, ClassDoc cls) {
        csv.append(String.format("%s;class\n", cls.name()));
        csv.append(String.format("%s;property;value\n", cls.name()));
    }
