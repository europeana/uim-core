...
public class MongoStorageEngine extends AbstractEngine implements StorageEngine<String> {
    ...
    public void initialize() {
        try {
            ...
            morphia.map(MongoProviderDecorator.class).map(MongoExecutionDecorator.class).map(
                    MongoCollectionDecorator.class).map(MongoRequestDecorator.class);
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#completed(eu.europeana.uim.api. ExecutionContext)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#updateProvider(eu.europeana.uim.store .Provider)
     */
    ...
    public void updateProvider(Provider<String> provider) throws StorageEngineException {
        ...
        ArrayList<MongoProviderDecorator> result1 = (ArrayList<MongoProviderDecorator>)ds.find(
                MongoProviderDecorator.class).filter(NAMEFIELD, provider.getName()).asList();
        ArrayList<MongoProviderDecorator> result2 = (ArrayList<MongoProviderDecorator>)ds.find(
                MongoProviderDecorator.class).filter(MNEMONICFIELD, provider.getMnemonic()).asList();
        ...
        for (MongoProviderDecorator<String> p : allresults) {
            if (p.getName() != null &&
                (p.getName().equals(provider.getName()) || p.getMnemonic().equals(
                        provider.getMnemonic())) && !p.getId().equals(provider.getId())) { throw new StorageEngineException(
                    "Provider with name '" + provider.getMnemonic() + "' already exists"); }
            if (p.getMnemonic() != null && p.getMnemonic().equals(provider.getMnemonic()) &&
                !p.getId().equals(provider.getId())) { throw new StorageEngineException(
    ...
    public Provider<String> getProvider(String id) {
        ...
        MongoProviderDecorator<String> prov = ds.find(MongoProviderDecorator.class).filter(
    ...
    public Provider<String> findProvider(String mnemonic) {
        MongoProviderDecorator<String> prov = ds.find(MongoProviderDecorator.class).field(
                MNEMONICFIELD).equal(mnemonic).get();
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#createCollection(eu.europeana.uim. store.Provider)
     */
    @SuppressWarnings("unchecked")
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#updateCollection(eu.europeana.uim. store.Collection)
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @Override
    public void updateCollection(Collection<String> collection) throws StorageEngineException {
        ...
        ArrayList<MongoCollectionDecorator> result1 = (ArrayList<MongoCollectionDecorator>)ds.find(
                MongoCollectionDecorator.class).filter(NAMEFIELD, collection.getName()).asList();
        ArrayList<MongoCollectionDecorator> result2 = (ArrayList<MongoCollectionDecorator>)ds.find(
                MongoCollectionDecorator.class).filter(MNEMONICFIELD, collection.getMnemonic()).asList();
        ...
        for (Collection<String> c : allresults) {
            if (c.getName() != null && (c.getName().equals(collection.getName())) &&
                !c.getId().equals(collection.getId())) { throw new StorageEngineException(
    ...
    public Collection<String> getCollection(String id) {
        @SuppressWarnings("rawtypes")
        MongoCollectionDecorator coll = ds.find(MongoCollectionDecorator.class).filter(
                LOCALIDFIELD, new ObjectId(id)).get();
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#getCollections(eu.europeana.uim.store .Provider)
     */
    ...
    public List<Collection<String>> getCollections(Provider<String> provider) {
        ...
        for (MongoCollectionDecorator<String> c : ds.find(MongoCollectionDecorator.class).filter(
                "provider", provider).asList()) {
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#createRequest(eu.europeana.uim.store .Collection,
     * java.util.Date)
     */
    @SuppressWarnings("unchecked")
    @Override
    public Request<String> createRequest(Collection<String> collection, Date date)
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#updateRequest(eu.europeana.uim.store .Request)
     */
    @Override
    public void updateRequest(Request<String> request) throws StorageEngineException {
        ...
        for (MongoRequestDecorator<?> r : ds.find(MongoRequestDecorator.class).filter("collection",
                request2.getCollectionReference()).asList()) {
            if (r.getDate().equals(request.getDate()) && !r.getId().equals(request2.getId())) {
                String unique = "REQUEST/" + request2.getCollection().getMnemonic() + "/" +
                                request2.getDate();
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#getRequests(eu.europeana.uim.store .Collection)
     */
    @SuppressWarnings("unchecked")
    @Override
    public List<Request<String>> getRequests(Collection<String> collection) {
        ...
        for (MongoRequestDecorator<String> r : ds.find(MongoRequestDecorator.class).filter(
    ...
    public Request<String> getRequest(String id) throws StorageEngineException {
        MongoRequestDecorator<?> req = ds.find(MongoRequestDecorator.class).filter(LOCALIDFIELD,
                new ObjectId(id)).get();
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#getRequests(eu.europeana.uim.store .MetaDataRecord)
     */
    ...
    public List<Request<String>> getRequests(MetaDataRecord<String> mdr)
        ...
        List<MongoRequestDecorator> results = ds.find(MongoRequestDecorator.class).filter(
                REQUESTRECORDS, mdr).asList();
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#createMetaDataRecord(eu.europeana.
     * uim.store.Collection, java.lang.String)
     */
    @SuppressWarnings("unchecked")
    @Override
    public MetaDataRecord<String> createMetaDataRecord(Collection<String> collection,
            String identifier) throws StorageEngineException {
        collection = (Collection<String>)ensureConsistency(collection);
        ...
        ds.save(mdr);
        mdr.getEmebeddedMdr().addValue(MONGOID, mdr.getMongoId().toString());
        ds.merge(mdr);
        return mdr.getEmebeddedMdr();
    }

    /*
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#addRequestRecord(eu.europeana.uim. store.Request,
     * eu.europeana.uim.store.MetaDataRecord)
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @Override
    public void addRequestRecord(Request<String> request, MetaDataRecord<String> record)
            throws StorageEngineException {
        MongoRequestDecorator<String> req = (MongoRequestDecorator<String>)ds.find(
                MongoRequestDecorator.class).filter(LOCALIDFIELD, new ObjectId(request.getId())).get();

        record = (MongoMetadataRecordDecorator)ensureConsistency(record);
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#updateMetaDataRecord(eu.europeana.
     * uim.store.MetaDataRecord)
     */
    @SuppressWarnings("unchecked")
    @Override
    public void updateMetaDataRecord(MetaDataRecord<String> record) throws StorageEngineException {
        record = (MongoMetadataRecordDecorator<String>)ensureConsistency(record);
        ds.merge(record);
    }
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#createExecution(eu.europeana.uim.store .UimDataSet,
     * java.lang.String)
     */
    @SuppressWarnings("unchecked")
    @Override
    public Execution<String> createExecution(UimDataSet<String> entity, String workflow)
            throws StorageEngineException {
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#updateExecution(eu.europeana.uim.store .Execution)
     */
    @SuppressWarnings("unchecked")
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#getMetaDataRecords(java.util.List)
     */
    @Override
    ...
    public MetaDataRecord<String> getMetaDataRecord(String id) throws StorageEngineException {
        ...
        @SuppressWarnings("unchecked")
        MongoMetadataRecordDecorator<String> request = ds.find(MongoMetadataRecordDecorator.class).filter(
                RECORDUIDFIELD, id).get();

        MetaDataRecord<String> returnRecord = (request == null) ? null : request.getEmebeddedMdr();
    ...
    public String[] getByRequest(Request<String> request) {
        ...
        @SuppressWarnings("unchecked")
        MongoRequestDecorator<String> cast = (MongoRequestDecorator<String>)ds.find(
                MongoRequestDecorator.class).filter(LOCALIDFIELD, new ObjectId(request.getId())).get();
    ...
    public String[] getByCollection(Collection<String> collection) {
        collection = (Collection<String>)ensureConsistency(collection);
        List<MongoMetadataRecordDecorator> reqrecords = ds.find(MongoMetadataRecordDecorator.class).filter(
                "collection", collection).asList();
        String[] res = new String[reqrecords.size()];
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#getByProvider(eu.europeana.uim.store.Provider,
     * boolean)
     */
    @SuppressWarnings({ "unchecked", "rawtypes" })
    @Override
    public String[] getByProvider(Provider<String> provider, boolean recursive) {
        ...
        List<MongoCollectionDecorator> mongoCollections = ds.find(MongoCollectionDecorator.class).filter(
                "provider", provider).asList();
     ...
     * 
     * @see
     * eu.europeana.uim.api.StorageEngine#getTotalByCollection(eu.europeana.uim.store.Collection)
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @Override
    public int getTotalByCollection(Collection<String> collection) {
        collection = (Collection<String>)ensureConsistency(collection);
        List<MongoMetadataRecordDecorator> records = ds.find(MongoMetadataRecordDecorator.class).filter(
                "collection", collection).asList();
     ...
     * (non-Javadoc)
     * 
     * @see eu.europeana.uim.api.StorageEngine#getTotalByProvider(eu.europeana.uim.store.Provider,
     * boolean)
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @Override
