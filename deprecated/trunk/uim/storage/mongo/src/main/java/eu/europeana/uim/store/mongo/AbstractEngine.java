...
public abstract class AbstractEngine {
     ...
     */
    public static final TKey<AbstractEngine, String> MONGOID = TKey.register(AbstractEngine.class,
                                                                     "mongoid", String.class);
    ...
    /**
     * Method that guarantees that the UIM Entity which is used for search purposes will always be
     * converted to MongoDB entity type which contains a valid ObjectID reference.
     * 
     * @param uimType
    ...
    UimEntity<?> ensureConsistency(UimEntity<?> uimType) {
        ...
        if (uimType instanceof RequestBean) {
            ...
            MongoRequestDecorator<String> requestDec = new MongoRequestDecorator<String>(colldec,
                    req.getDate());
        ...
        if (uimType instanceof MetaDataRecordBean) {
            ...
            MongoMetadataRecordDecorator<String> mdrdec = new MongoMetadataRecordDecorator<String>(
                    colldec, mdr.getId());
        ...
        if (uimType instanceof ExecutionBean) {
            ...
            MongoExecutionDecorator<String> exdec = new MongoExecutionDecorator<String>(
                    exbean.getId());
     ...
     * @return
     */
    private MongoProviderDecorator<String> convertProviderBean2Decorator(
            ProviderBean<String> provbean, boolean recursive) {
        ...
        if (recursive == true) {
            ...
            while (initerator.hasNext()) {
                MongoProviderDecorator<String> tmpprovdec = convertProviderBean2Decorator(
                        (ProviderBean<String>)initerator.next(), false);
            ...
            while (outiterator.hasNext()) {
                MongoProviderDecorator<String> tmpprovdec = convertProviderBean2Decorator(
                        (ProviderBean<String>)outiterator.next(), false);
     ...
     * @return
     */
    private MongoCollectionDecorator<String> convertCollectionBean2Decorator(
            CollectionBean<String> collbean) {
        MongoProviderDecorator<String> provdec = convertProviderBean2Decorator(
                (ProviderBean<String>)collbean.getProvider(), true);
