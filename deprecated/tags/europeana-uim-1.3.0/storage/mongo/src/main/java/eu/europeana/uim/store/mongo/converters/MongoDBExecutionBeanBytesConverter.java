...
public class MongoDBExecutionBeanBytesConverter extends
		Converter<byte[], ExecutionBean> {

	/**
	 * logging variable for execution converter
	 */
	private static final Logger log = Logger
			.getLogger(MongoDBExecutionBeanBytesConverter.class.getName());
			...
			try {
				while ((tag = input.readTag()) != 0) {
					...
					case STARTTIME:
						...
						if (time != null && time.trim().length() > 0) {
							try {
								...
								log.severe("Failed to parse start: <" + time
										+ "> as date.");
							}
						}
						break;
					...
					case PROCESSED:
						...
						break;
					case VALUES:
						Map<String, String> map = MongoDBCollectionBeanBytesConverter
								.decodeMap(input.readString());
	...
	public byte[] encode(ExecutionBean data) {
		...
		try {
			...
			if (data.values() != null && !data.values().isEmpty()) {
				output.writeString(VALUES, MongoDBCollectionBeanBytesConverter
						.encodeMap(data.values()));
			}
