...
public class MemoryLoggingEngine<T> implements LoggingEngine<T> {
    private Map<?, List<LogEntry<?, String[]>>> executionLogs           = new HashMap<I, List<LogEntry<I, String[]>>>();
    private Map<?, List<LogEntry<?, T>>>        structuredExecutionLogs = new HashMap<I, List<LogEntry<I, T>>>();
    ...
    @Override
    public void log(String module, Execution<?> execution, String scope, Level level,
            String... message) {
        ...
        logs.add(new MemoryLogEntry<String[]>(module, (Execution<Long>)execution, scope, level, new Date(),message));
    }

    @Override
    public void log(IngestionPlugin plugin, Execution<?> execution, MetaDataRecord<?> mdr,
        ...
        if (logs == null) {
            ...
            executionLogs.put((I)execution.getId(), logs);
        }
        logs.add(new MemoryLogEntry<I, String[]>(plugin, (Execution<I>)execution, (MetaDataRecord<I>)mdr, scope, level, new Date(),
                message));
    }

    @Override
    public void logStructured(IngestionPlugin plugin, Execution<?> execution,
            MetaDataRecord<?> mdr, String scope, Level level, T payload) {
        List<LogEntry<I, T>> logs = structuredExecutionLogs.get(execution);
        if (logs == null) {
            logs = new ArrayList<LogEntry<I, T>>();
            structuredExecutionLogs.put((I)execution.getId(), logs);
        }
        logs.add(new MemoryLogEntry<I, T>(plugin, (Execution<I>)execution, (MetaDataRecord<I>)mdr, scope, level, new Date(), payload));
    }

    @Override
    public <I> List<LogEntry<I, String[]>> getExecutionLog(Execution<I> execution) {
        return executionLogs.get(execution.getId());
    }

    @Override
    public <I> List<LogEntry<I, String[]>> getExecutionLog(I executionID) {
        return executionLogs.get(executionID);
    }

    @Override
    public <I> List<LogEntry<I, T>> getStructuredExecutionLog(Execution<I> execution) {
        return structuredExecutionLogs.get(execution.getId());
    }

    @Override
    public <I> List<LogEntry<I, T>> getStructuredExecutionLog(I executionID) {
    ...
    @Override
    public <I> void logDurationDetailed(IngestionPlugin plugin, Long duration, I... mdrs) {
