...
public class MongoDBCollectionBeanBytesConverter extends
		Converter<byte[], CollectionBean> {
	 ...
	 * 
	 * @see
	 * org.theeuropeanlibrary.repository.convert.Converter#decode(java.lang.
	 * Object)
	 */
	@Override
	public CollectionBean<String> decode(byte[] data) {
		...
		try {
			...
			throw new RuntimeException(
					"Could not read ProviderBean from byte array!", e);
		} finally {
		}
		return bean;
	}
	 ...
	 * 
	 * @see
	 * org.theeuropeanlibrary.repository.convert.Converter#encode(java.lang.
	 * Object)
	 */
	...
	public byte[] encode(CollectionBean data) {
		...
		try {
			...
			throw new RuntimeException("Could not write ProviderBean '"
					+ data.getId() + "' to byte array!", e);
		} finally {
			try {
				bout.close();
			} catch (IOException e) {
	...
	/**
	 * Encodes the values contained in a Collection Map to a String
	 * Representation
	 * 
	 * @param data
	 *            the Map to encode
	 * @return the encoded map to a String
	 */
	...
	/**
	 * Decodes a String to a Collection Map Representation
	 * 
	 * @param data
	 *            the String to decode
	 * @return the Map containing values
	 */
			...
			for (String string : split) {
				...
				if (split2.length != 2) {
					throw new IllegalArgumentException(
							"Can not parse map from string: <" + string + ">");
				} else {
					String stripKey = StringUtils.strip(split2[0], "\"");
					if (stripKey.equals(split2[0]))
						stripKey = null;

					String stripVal = StringUtils.strip(split2[1], "\"");
					if (stripVal.equals(split2[1]))
						stripVal = null;
