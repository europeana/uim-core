...
import org.theeuropeanlibrary.uim.check.edm.AbstractEdmIngestionPlugin;
import org.theeuropeanlibrary.uim.check.edm.EdmCheckIngestionPlugin;
...
public class EdmCheckIngestionPluginTest {
    ...
    public void testSimpleCheck() throws Exception {
        ...
        ContextRunningData data = new AbstractEdmIngestionPlugin.ContextRunningData(
                new File("src/test/resources/xsd/EDM_tel.xsd"));
        data.maxErrors=10;
        
        when(context.getValue((TKey<?, ContextRunningData>)any())).thenReturn(data);
        when(context.getFileResource((String)any())).thenReturn(new File("src/test/resources/xsd/EDM_tel.xsd"));
        
        plugin.initialize(context);
        IomXmlReader reader=new IomXmlReader(new File("src/test/resources/sample_a0232.teliom.xml"));
