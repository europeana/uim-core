...
public class TelInternalDocumentationDoclet {
    ...
    static {
        try {
            InputStream textStream = TelInternalDocumentationDoclet.class.getClassLoader().getResourceAsStream("org/theeuropeanlibrary/model/tel/documentation/text_content.txt");
    ...
    public static boolean start(RootDoc root) {
        ...
        for (int i = 0; i < classes.length; ++i) {
            ...
            if(className.contains(".qualifier.")) {
                ...
                if(group.equals("common") || group.equals("tel"))
                    group="";
...
//            trans.setOutputProperty(OutputKeys.OMIT_XML_DECLARATION, "yes");
    ...
    private static void createIntroductoryText(DomBuilder html) {
        for(String section: new String[] {"Introduction", "Why Internal?", "Function and Purpose", "Supported Data Formats", "Known Limitations", "About the Model's Structure" }) {
    ...
    private static void createIndex(DomBuilder html, HashMap<String, ArrayList<ClassDoc>> classesByGroup, 
            ArrayList<ClassDoc> qualifiers, HashSet<ClassDoc> superClasses) {
    ...
    private static boolean isClassToDocument(ClassDoc cls, HashSet<ClassDoc> superClasses) {
        ...
        if(tkey==null && !superClasses.contains(cls))
            return false;
        return true;
    }
    ...
    private static void classToDom(DomBuilder dom, ClassDoc cls, HashSet<String> allClasseNames) {
        ...
        boolean hasSuperClass=cls.superclass()!=null && !cls.superclass().name().equals("Object") && !cls.superclass().name().equals("Enum");
        if(hasSuperClass) {
            dom.addElement("tr");
     ...
     */
    private static void addClassFields(DomBuilder dom, ClassDoc cls, boolean hasSuperClass, HashSet<String> allClasseNames) {
        dom.addElement("table");
        dom.setAttribute("border", 1);
        dom.addElement("tr");
        dom.addElement("td");
